...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        count = 0
        while not current_place.is_hive and count < self.lower_bound:
            current_place = current_place.entrance
            count += 1
        while not current_place.is_hive and count <= self.upper_bound:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while current_place is not None:
            if current_place.ant and not current_place.ant.doubled:
                current_place.ant.double()
            if current_place.ant and current_place.ant.is_container and current_place.ant.ant_contained and not current_place.ant.ant_contained.doubled:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target is not None:
            target.slow_turns = 5
            if not hasattr(target, 'has_been_slowed'):
                prev_action = target.action
                def action(gamestate):
                    original_slow_turns = target.slow_turns
                    if original_slow_turns <= 0 or gamestate.time % 2 == 0:
                        prev_action(gamestate)
                    target.slow_turns = original_slow_turns - 1
                target.action = action
                target.has_been_slowed = True
        # END Problem EC 1
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if target and not hasattr(target, 'scare_length'):
            target.scare(2)
        # END Problem EC 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        for bee in self.place.bees:
            result[bee] = 0
        if self.place.ant is not self and self.place.ant is not None:
            result[self.place.ant] = 0
        current_place, distance = self.place.entrance, 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        result = self.damage - self.insects_shot * 0.0625 - distance * 0.25
        if result <= 0:
            return 0
        return result
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if hasattr(self, 'scare_length') and self.scare_length > 0:
