...
def accuracy(typed: str, source: str) -> float:
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    num, don = 0, len(typed_words)
    if don == 0:
        if source_words:
            return 0.0
        else:
            return 100.0
    for i in range(min(don, len(source_words))):
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        imumutable_args = (typed, source)
        if imumutable_args not in cache or limit > cache[imumutable_args][1]:
            value = diff_function(typed, source, limit)
            cache[imumutable_args] = (value, limit)
            return value
        return cache[imumutable_args][0]
        # END PROBLEM EC
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    word_min_diff = word_list[0]
    for i in range(1, len(word_list)):
        diff = diff_function(typed_word, word_list[i], limit)
        if diff < min_diff:
            min_diff = diff
            word_min_diff = word_list[i]
    if min_diff > limit:
        return typed_word
    else:
        return word_min_diff
    # END PROBLEM 5
...
def minimum_mewtations(typed: str, source: str, limit: int) -> int:
    ...
    if len(source) == 0 or len(typed) == 0: # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def time_per_word(words: list[str], timestamps_per_player: list[list[int]]) -> dict:
    ...
    # BEGIN PROBLEM 9
    times = [[timestamps[i + 1] - timestamps[i] for i in range(len(timestamps) - 1)] for timestamps in timestamps_per_player]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    for i in word_indices:
        count = get_time(times, 0, i)
        winner = 0
        for j in player_indices[1: ]:
            if get_time(times, j, i) < count:
                winner = j
                count = get_time(times, j, i)
        result[winner] += [words[i]]
    return result
    # END PROBLEM 10
